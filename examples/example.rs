use hurrahdb::{Storage, AofConfig, Config, Type};
...
#[tokio::main]
async fn main() {
    let storage = match Storage::new(Some(Config { aof_config: Some(AofConfig { sync_time: 100, file_name: "example_file".to_string() }), persistance_type: Type::Aof })) {
        Ok(s) => s,
        Err(err) => {
    ...
    let model1 = TestModel1{name: "some-value-1".to_string()};
    let model2 = TestModel2{value: "some-value-2".to_string()};
